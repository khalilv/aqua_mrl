...
from aqua_mrl.helpers import reward_calculation, random_starting_position, adv_mapping
    ...
    def finish(self):
        ...
        if self.episode < self.stop_episode:
            ...
            if self.episode % self.switch_every == 0:
                print('-------------- Switching to protagonist --------------')
                subprocess.Popen('python3 ./src/aqua_mrl/aqua_mrl/resetter.py', shell=True)
            else:
                subprocess.Popen('python3 ./src/aqua_mrl/aqua_mrl/resetter_adv.py', shell=True)
