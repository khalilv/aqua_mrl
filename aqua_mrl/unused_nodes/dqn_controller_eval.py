...
from aqua_mrl.control.DQN import DQN
...
class dqn_controller_eval(Node):
    def __init__(self):
        ...
        if self.eval_episode == -1:
            self.checkpoint_path = 'src/aqua_mrl/experiments/{}/best.pt'.format(str(self.experiment_number))
            self.save_path = 'src/aqua_mrl/evaluations/{}_episode_best/'.format(str(self.experiment_number))
        else:
            self.checkpoint_path = 'src/aqua_mrl/experiments/{}/weights/episode_{}.pt'.format(str(self.experiment_number), str(self.eval_episode).zfill(5))
            self.save_path = 'src/aqua_mrl/evaluations/{}_episode_{}/'.format(str(self.experiment_number), str(self.eval_episode).zfill(5))
